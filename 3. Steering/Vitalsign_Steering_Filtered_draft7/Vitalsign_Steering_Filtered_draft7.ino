...
const byte RED_LED_LOW = 0x00;
void sendFinger();   // Intensitas rendah
  ...
  while (true) {
    ...
    if (currentMillis - lastSampleTime >= SAMPLE_INTERVAL) {
      ...
      if (mafIR < 40000 || mafRed == 0 ) {
        ...
        scanMessageDisplayed2 = false;
        particleSensor.setPulseAmplitudeRed(RED_LED_LOW);
        //Serial.println("Place Finger");
        if (!jarinempel) { // Hanya kirim pesan jika sebelumnya jari terdeteksi
          Pesan = "Place Finger"; // Update pesan
          sendFinger(); // Kirim pesan ini segera
          jarinempel = true; // Reset flag
        }
        ...
        // Reset flag ketika jari sudah menempel
        if (jarinempel) {
          jarinempel = false;  // Reset flag setelah jari terdeteksi
        }
      }
...
void sendFinger()
   if (pRemoteCharacteristic != nullptr && pClient != nullptr && pClient->isConnected() && pRemoteCharacteristic->canWrite()) {
    String finger = Pesan ; // Tambahkan prefix "MSG:" untuk identifikasi di sisi penerima (Flutter)
    pRemoteCharacteristic->writeValue(finger.c_str());
