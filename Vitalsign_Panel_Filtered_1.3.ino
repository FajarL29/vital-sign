...
class MyCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic *pCharacteristic) {
    ...
    Serial.println("[BLE WRITE] Data diterima: " + value);
  }
};
...
void calculateHeartRate(long irVal) {
  if (checkForBeat(irVal)) {
    ...
    //beatAvg = weightedSum;
    float weight = 0.6;
    beatAvg = (weight * beatsPerMinute) + ((1 - weight) * beatAvg) + 15;
...
void calculateSpO2(long irVal, long redVal) {
  ...
  if (first) {
    ...
    irDC = 0.9 * irDC + 0.1 * irVal;
    redDC = 0.9 * redDC + 0.1 * redVal;
  }

  long irAC = irVal - irDC;
  long redAC = redVal - redDC;
